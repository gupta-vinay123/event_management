...
const UserDashboard = () => {
  ...
  const handleDownloadTicket = async (bookingId) => {
    try {
      const response = await API.get(`/bookings/download/${bookingId}`, {
        responseType: 'blob',
      });
        ...
        <main className="ml-64 flex-grow p-10 pb-20 relative">
          ...
          <header className="flex justify-between items-center mb-12">
            <div className="text-left">
              {/* ✅ user comes from context, not JSON.parse(localStorage.getItem('userInfo')) */}
          ...
          <section className="mb-20">
            ...
            <div className="bg-white/5 border border-white/10 rounded-[2.5rem] overflow-hidden backdrop-blur-md">
              {bookings.length === 0 ? (
                <div className="p-20 text-center text-gray-500 italic">No bookings have been made yet.</div>
              ) : (
