...
import { MapPin, DollarSign, Type, AlignLeft, Clock, Calendar } from 'lucide-react';
...
const EditEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = errData?.errors?.join('\n') || errData?.message || 'Update failed. Please try again.';
      alert(msg);
    }
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto pb-10">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white/5 p-8 rounded-[2.5rem] border border-white/10 backdrop-blur-md shadow-2xl">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative mt-2">
                ...
                <input type="date" value={formData.date} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-red-500" onChange={(e) => setFormData({...formData, date: e.target.value})} required />
              </div>
            </div>
            <div>
              ...
              <div className="relative mt-2">
                ...
                <input type="time" value={formData.startTime} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-red-500" onChange={(e) => setFormData({...formData, startTime: e.target.value})} required />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-gray-400 text-xs font-bold uppercase ml-1">Price (LKR)</label>
              <div className="relative mt-2">
                <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500" />
                <input type="number" value={formData.price} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-red-500" onChange={(e) => setFormData({...formData, price: e.target.value})} required />
              </div>
            </div>
            <div>
